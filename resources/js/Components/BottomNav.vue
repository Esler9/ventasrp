<template>
    <nav class="fixed inset-x-0 bottom-0 z-40 bg-gray-900/95 backdrop-blur border-t border-gray-800" :style="safeAreaStyle">
        <div class="mx-auto flex max-w-6xl items-center justify-around px-4 pt-3 pb-4 text-xs text-gray-300">
            ...
            <a :href="routes.products" class="flex flex-col items-center gap-1" :class="isActive('products')">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="m4 7 8-4 8 4m-14 0v10l6 3m0-13 6-3v10l-6 3m0-13v13" />
                </svg>
                <span>Productos</span>
            </a>
            <a :href="routes.sales" class="flex flex-col items-center gap-1" :class="isActive('sales')">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            <a :href="routes.panel" class="flex flex-col items-center gap-1" :class="isActive('panel')">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
const isActive = (key) => {
    const path = page.url || '';
    if (key === 'pos') return path.startsWith('/pos');
    if (key === 'products') return path.startsWith('/admin/products');
